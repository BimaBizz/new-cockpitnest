// Fetch social media menu using fetchMenu from pages.js
export const getSosmedMenu = async (options = {}) => {
  // Return the full menu object (with .links)
  const menu = await fetchMenu("sosmed", options).catch(() => null);
  return menu || null;
};
import {
  MODELS,
  PRO_PAGES_DEFAULT_MENU,
  PRO_PAGES_ENABLED,
} from "@/config/cockpit";
